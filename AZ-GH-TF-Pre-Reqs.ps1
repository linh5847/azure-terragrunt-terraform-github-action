...
# Setup Variables.
$environment = "dev"
$upperc = "$environment".ToUpper()
$randomInt = Get-Random -Maximum 9999
$subscriptionId=$(az account show --query id -o tsv)
$resourceGroupName = "Terraform-$environment-Backend-RG"
$storageName = "tf$environmentbackendsa$randomInt"
$kvName = "tf-$environment-backend-kv$randomInt"
$appName="tf-$environment-github-SPN$randomInt"
$region = "uksouth"
...
foreach($object_properties in $spnObj.psobject.properties) {
    If ($object_properties.Name -eq "appId") {
        $null = az keyvault secret set --vault-name $kvName --name "$upperc-ARM-CLIENT-ID" --value $object_properties.Value
    }
    If ($object_properties.Name -eq "password") {
        $null = az keyvault secret set --vault-name $kvName --name "$upperc-ARM-CLIENT-SECRET" --value $object_properties.Value
    }
    If ($object_properties.Name -eq "tenant") {
        $null = az keyvault secret set --vault-name $kvName --name "$upperc-ARM-TENANT-ID" --value $object_properties.Value
    }
}
$null = az keyvault secret set --vault-name $kvName --name "$upperc-ARM-SUBSCRIPTION-ID" --value $subscriptionId
